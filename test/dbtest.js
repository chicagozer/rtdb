...
describe('Suite', function() {
	...
	describe('Database', function() {
		...
		it('Create a view', function(done) {
			v = new View(db,c).init('junk', 'emit(item.name,item.value)',
		...
		it('add 200 documents', function(done) {
			var i=0;
			async.whilst(function() { return (i++ < 200);},
					function (callback) { 
						var doc = [ {
							name : 'test3',
							value : 1
		...
		it('Create a second view', function(done) {
			v2 = new View(db,c).init('junk2', 'emit(item.name,item.value)',
					'emit(values.reduce(function (a, b) { return a + b;}));',
					'reduction.sort(function(a,b){return b[1] -a[1];});emit(reduction);');
			c.addView(v2,done);
		});
